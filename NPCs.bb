...
Const NPCtype106Victim% = 23, NPCtypeScientist% = 24, NPCtypeFranklin% = 25, NPCtypeJanitor% = 26, NPCtypeGonzales% = 27
Const NPCtypeCorpse% = 28, NPCtypeNazi% = 29, NPCtypeZombie2% = 30, NPCtype457% = 31, NPCtypeMTF2% = 32
Const NPCtypeVehicle% = 33, NPCtypeD1% = 34, NPCtypeD2% = 35
;[End Block]
...
Function CreateNPC.NPCs(NPCtype%, x#, y#, z#)
	...
	Select NPCtype
		...
		Case NPCtypeOldMan
			;[Block]
			n\NVName = "XENOTHERIUM SUBSERVILIA"
			n\Collider = CreatePivot()
			n\GravityMult = 0.0
			n\MaxGravity = 0.0
		...
		Case NPCtypeD
			...
			temp# = (GetINIFloat("DATA\NPCs.ini", "Scientist", "scale") / 2.4)
			ScaleEntity n\obj, temp, temp, temp
		...
		Case NPCtypeD1
			...
			n\obj = CopyEntity(o\NPCModelID[44])
		...
		Case NPCtypeD2
			...
			n\obj = CopyEntity(o\NPCModelID[45])
		...
		Case NPCtype5131
			...
			temp# = 0.4 / MeshWidth(n\obj)
			ScaleEntity n\obj, temp, temp, temp
			ScaleEntity n\obj2, temp, temp, temp
			;[End Block]
		Case NPCtype096
			;[Block]
			n\NVName = "XENOTHERIUM MYRMEX"
			n\Collider = CreatePivot()
		...
		Case NPCtypeApache
			...
			n\obj2 = CopyEntity(o\NPCModelID[4])
			...
			n\obj3 = CopyEntity(o\NPCModelID[5])
			;PositionEntity n\obj3, 0.0, 2.15, -5.48
			PositionEntity n\obj3, -0.1, 1.75, -5.1
			...
			For i = -1 To 1 Step 2
				...
				PositionEntity(Light1, 2.3*i, 1.1, -0.25)
				
				Local lightsprite = CreateSprite(n\obj)
				;PositionEntity(lightsprite, 1.65*i, 1.17, 0, -0.25)
				PositionEntity(lightsprite, 2.3*i, 1.1, -0.25, -0.25)
		...
		Case NPCtype939
			...
			n\NVName = "GASTROPOLYPUS TOXOPHLEGMATA"
		...
		Case NPCtype066
			;[Block]
			n\NVName = "XENOTHERIUM MONOPSCELIDA"
			n\Collider = CreatePivot()
			EntityRadius n\Collider, 0.2
			EntityType n\Collider, HIT_PLAYER
		...
		Case NPCtype1499
			;[Block]
			n\NVName = "XENOTHERIUM MYRMELEONTIDAE"
			n\Collider = CreatePivot()
			EntityRadius n\Collider, 0.2
			EntityType n\Collider, HIT_PLAYER
			For n2.NPCs = Each NPCs
		...
		Case NPCtype106Victim
			...
			n\obj = CopyEntity(o\NPCModelID[30])
        ...
        Case NPCtypeFranklin
         ...
         Case NPCtypeScientist
			...
			n\obj = CopyEntity(o\NPCModelID[35])
		...
		Case NPCtype457
		    ;[Block]
			n\NVName = "SACTILDIAE NEFARIOUSIUM"
			n\Collider = CreatePivot()
			EntityRadius n\Collider, 0.2
			EntityType n\Collider,HIT_PLAYER
			
            If n\obj = 0 Then
			    n\obj = CopyEntity(o\NPCModelID[41])
        ...
        Case NPCtypeZombie2
			...
			If n\obj = 0 Then 
				...
				n\obj = CopyEntity(o\NPCModelID[36])
		...
		Case NPCtypeMTF2
			...
			If n\obj = 0 Then
			    n\obj = CopyEntity(o\NPCModelID[42])
			...
			If MTF2SFX(0) = 0
			;    MTF2SFX(0) = LoadSound_Strict("SFX\Character\MTF2\Stop1.ogg")
			;    MTF2SFX(1) = LoadSound_Strict("SFX\Character\MTF2\Stop2.ogg")
			;    MTF2SFX(2) = LoadSound_Strict("SFX\Character\MTF2\Stop3.ogg")			
			;	 MTF2SFX(3) = LoadSound_Strict("SFX\Character\MTF2\ClassD2.ogg")
			;	 MTF2SFX(4) = LoadSound_Strict("SFX\Character\MTF2\ClassD1.ogg")
				 MTF2SFX(5) = LoadSound_Strict("SFX\Character\MTF2\Beep.ogg")
			EndIf
		...
		Case NPCtypeVehicle
            ...
            If n\obj = 0 Then
			    n\obj = CopyEntity(o\NPCModelID[43])
				
			    temp# = 2.3 / MeshWidth(n\obj)
			    ScaleEntity n\obj, temp, temp, temp
	...
	For n.NPCs = Each NPCs
		...
		Select n\NPCtype
											...
											If KillTimer >= 0 And (Not GodMode) Then
												...
												Select PlayerRoom\RoomTemplate\Name
													Case "lockroom", "room2closets", "coffin"
														...
														DeathMSG = DeathMSG + "POST-MORTEM: Subject was killed by BMR-500."	
													Case "173"
														DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
														DeathMSG = DeathMSG + "POST-MORTEM: Subject was killed by BMR-500 during the onset of disaster."
													Case "room2doors"
														DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
														DeathMSG = DeathMSG + "POST-MORTEM: Subject was killed by BMR-500."
													Default 
			...
			Case NPCtypeOldMan ;------------------------------------------------------------------------------------------------------------------
				...
				Else
					...
					If (Not n\Idle) And spawn106%
									...
									If KillTimer >= 0 And FallTimer >= 0 Then
										...
										If Ceil(n\Frame) = 110 And (Not GodMode) Then
											...
											If PlayerRoom\RoomTemplate\Name = "pocketdimension" Then
												DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
												DeathMSG = DeathMSG + "POST-MORTEM: Subject was captured and killed by Xenian hostiles."
												Kill()
											Else
						...
						MoveEntity(n\obj2, 0, 5.3 * 0.11, -5.5 * 0.11)
									...
									Else
										...
										If n\Target=Null Then
											If (Not GodMode) Then 
												...
												BlurTimer = 2000
												DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
												DeathMSG = DeathMSG + "POST-MORTEM: Subject was messily killed and devoured by Xenian hostile."
												Kill()
												KillAnim = 1
												For i = 0 To 6
			...
			Case NPCtype049
									...
									If dist < 0.5 Then
										...
										Else
											...
											If (Not GodMode) Then
												If PlayerRoom\RoomTemplate\Name$ = "room049"
													DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
													DeathMSG = DeathMSG + "POST-MORTEM: Subject was lacerated and mutated into Xenian hostile. Terminated by military forces."
													For e.events = Each Events
				...
				;[End Block]
			Case NPCtypeZombie, NPCtypeZombie2
				;[Block]
									...
									If prevFrame < 23 And n\Frame=>23 Then
										If EntityDistance(n\Collider,Collider)<1.1
											If (Abs(DeltaYaw(n\Collider,Collider))<=60.0)
												...
												EndIf
												DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
												DeathMSG = DeathMSG + "POST-MORTEM: Subject was lacerated and partially consumed by Xenian hostile."
											EndIf
										EndIf
				...
				Select n\State
							...
							If dist < DetectDistance Then
								...
								If n\Reload = 0 ;And n\Frame>1550 Then
									...
									If PickedEntity() = Collider Or n\State3=1 Then
										...
										PlaySound2(Gunshot1SFX, Camera, n\Collider, 35)
					...
					Case 2 ;shoots
						...
						If n\Reload = 0 ;And n\Frame > 1545 Then 
							PlaySound2(Gunshot1SFX, Camera, n\Collider, 20)
			...
			Case NPCtypeMTF ;------------------------------------------------------------------------------------------------------------------
				...
				;[End Block]
			Case NPCtypeD, NPCtypeD1, NPCtypeD2, NPCtypeClerk, NPCtype106Victim, NPCtypeScientist, NPCtypeGonzales, NPCtypeFranklin, NPCtypeJanitor, NPCtypeCorpse, NPCtype035, NPCtypeNazi 	;------------------------------------------------------------------------------------------------------------------
				;[Block]
				...
				Select n\State
					Case 0 ;idle
						...
						Animate2(n\obj, AnimTime(n\obj), 211, 235, 0.1)
					Case 1 ;walking
						If n\State2 = 1.0
							n\CurrSpeed = CurveValue(n\Speed*0.7, n\CurrSpeed, 20.0)
						Else
										...
										If n\Reload =< 0 Then
											If dist<20.0 Then
												...
												If WrapAngle(EntityYaw(pvt)-EntityYaw(n\Collider))<10 Then
													...
													If PlayerRoom\RoomTemplate\Name = "exit1" Then
														DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
     													DeathMSG = DeathMSG + "POST-MORTEM: Subject was shot and killed by patrolling helicopter before reaching an objective."
													Else
														DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
     													DeathMSG = DeathMSG + "POST-MORTEM: Subject was shot and killed by patrolling helicopter before reaching an objective."
													EndIf
											...
											Else
												...
												If Injuries > 3.0 Then
													If PlayerRoom\RoomTemplate\Name = "room2offices" Then
														DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
														DeathMSG = DeathMSG + "POST-MORTEM: Subject attacked and killed by stationary Xenian hostile."
													Else
														DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
														DeathMSG = DeathMSG + "POST-MORTEM: Subject attacked and killed by stationary Xenian hostile."
													EndIf
				...
				If PlayerRoom\RoomTemplate\Name = "room860" Then
					...
					If n\State <> 0 Then
						...
						If dist > 8.0 Then
							...
							MoveEntity(n\obj2, 0, 70.0*0.025, -50.0*0.025)
							...
							If n\LastSeen > 0 And (Not NoTarget) Then
								...
								If (n\Frame=>18.0 And n\Frame<68.0) Then
									...
									If Injuries>4.0 Then 
										DeathMSG="SUBJECT: WALKER | STATUS: DEAD | "
										DeathMSG=DeathMSG+"POST-MORTEM: Subject was killed and consumed by aggressive Xenian hostile."
										Kill()
				...
				If (dist<HideDistance) Then
					...
					If (WearingNightVision=0) Then
						...
						If dist<1 And n\Reload <= 0 And MsgTimer <= 0 Then
							Select Rand(6)
								Case 1
									Msg="You hear strange voices right next to you."
								Case 2
...
;								EndIf
									...
									Else
										...
										If Injuries > 10.0
											...
											If PlayerRoom\RoomTemplate\Name$ = "dimension1499"
												DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
												DeathMSG = DeathMSG + "POST-MORTEM: An HEV scientist with Displacer Cannon was found dead on the one of Xen islands. "
												DeathMSG = DeathMSG + "Seems like he was attacked by Xenian hostiles or another creatures."
											Else
												DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
												DeathMSG = DeathMSG + "POST-MORTEM: An aggressive alien creature was found in [REDACTED], consuming the corpse of HEV scientist. "
			...
			Case NPCtype457
			    ...
			    If PlayerRoom\RoomTemplate\Name <> "room457"
					n\State = 3   
				EndIf

				prevFrame# = n\Frame
				
				If n\State < 3 Then ;it used if player escaped from the room457 tunnels
				    If (Not Contained457)
				        Select n\State
					        Case 0 ;normal routine (walking and idling)
						        ...
						        If EntityDistance(n\Collider,Collider) < 6.0 And (Not Notarget)
							        If EntityVisible(n\Collider,Collider)
								        n\LastSeen = 500
								        n\State = 1
								        n\PathStatus = 0
								        ...
								        If EntityVisible(n\Collider, Collider)
									        ...
									        If EntityDistance(n\Collider,Collider) < 0.6
										        ...
										        If KillTimer >= 0 And (Not GodMode) Then
										            ...
										            RotateEntity n\Collider, 0, CurveAngle(EntityYaw(n\obj), EntityYaw(n\Collider), 10.0), 0 
										            If Ceil(n\Frame) = 110 And (Not GodMode) Then
										                PlaySound_Strict(LoadTempSound("SFX\SCP\457\Attack.ogg"))
												        Kill()
												
											            DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
														DeathMSG = DeathMSG + "POST-MORTEM: Subject was killed by Xenian Kingpin."
	                                               EndIf
	                                            EndIf
										    EndIf
									    EndIf
                    ...
                    If prevFrame =< 286 And n\Frame>286 Then
				    	PlaySound2(LoadTempSound("SFX\SCP\457\Walk.ogg"),Camera, n\Collider, 6.0, Rnd(0.8,1.0))	
				    ElseIf prevFrame=<311 And n\Frame>311.0 
					    PlaySound2(LoadTempSound("SFX\SCP\457\Walk.ogg"),Camera, n\Collider, 6.0, Rnd(0.8,1.0))
				    EndIf
				EndIf
			...
			Case NPCtypeVehicle 
				...
				Select n\State
					Case 0 ;idle
					    n\CurrSpeed = CurveValue(0.0, n\CurrSpeed, 5.0)
						AnimateNPC(n, 21, 40, n\CurrSpeed * 18)
					Case 1 ;walking
						n\CurrSpeed = CurveValue(n\Speed*0.7, n\CurrSpeed, 20.0)
						AnimateNPC(n, 1, 20, n\CurrSpeed * 18)
						
						If n\CurrSpeed > 0.005 Then	
						    If (prevFrame < 10 And n\Frame=>10) Or (prevFrame < 20 And n\Frame=>20)	Or (prevFrame < 5 And n\Frame=>5)	
						        PlaySound2(VehicleSFX, Camera, n\Collider, 15)	
						    EndIf
					    EndIf		
				End Select
						...
						If n\Reload =< 0 And KillTimer = 0 Then
							If EntityVisible(n\Collider, Camera) Then
								...
								If angle < 5 Or angle > 355 Then 
									...
									Shoot(EntityX(pvt),EntityY(pvt), EntityZ(pvt),((20/dist)*(1/dist)), True)
									n\Reload = 7
									
									FreeEntity(pvt)
									
									DeathMSG="SUBJECT: WALKER | STATUS: DEAD | "
									DeathMSG=DeathMSG+"POST-MORTEM: Subject died of bloodloss following assault by HECU Marines."
									
									;player killed -> "target terminated"
									If prev => 0 And KillTimer < 0 Then
										DeathMSG="SUBJECT: WALKER | STATUS: DEAD | "
										DeathMSG=DeathMSG+"POST-MORTEM: Terminated by Hazardous Environment Combat Unit."
						...
						If (Abs(DeltaYaw(n\Collider,n\Target\Collider))<50.0)
							...
							Else
								If (Not n\Target\IsDead)
									If n\Sound <> 0 Then FreeSound_Strict n\Sound : n\Sound = 0
									If n\NPCtype = NPCtypeZombie Or NPCtypeZombie2
										n\Sound = LoadSound_Strict("SFX\Character\MTF\049\Player0492_2.ogg")
										PlayMTFSound(n\Sound, n)
										SetNPCFrame(n\Target,133)
									ElseIf n\NPCtype = NPCtype008
										n\Sound = LoadSound_Strict("SFX\Character\MTF\049\Player0492_2.ogg")
										PlayMTFSound(n\Sound, n)
										SetNPCFrame(n\Target,346)
									EndIf
								EndIf
								;SetNPCFrame(n\Target,133)
...
Function UpdateMTF2Unit(n.NPCs)
	...
	If n\BlinkTimer<=-5.0 Then 
		...
		;	    PlayMTFSound(LoadTempSound("SFX\Character\MTF2\173\BLINKING"+Rand(1,4)+".ogg"),n)
		;	EndIf
		;EndIf
		n\BlinkTimer = 70.0*Rnd(10.0,15.0)
	EndIf	
		...
		Select Int(n\State) ;what is this MTF2 doing
			Case 0 ;wandering around
								...
								For r = Each Rooms
									If r\RoomTemplate\Name$ = "start"
										...
										Else
											...
											Curr173\IsDead = True
											;If n\Sound <> 0 Then FreeSound_Strict n\Sound : n\Sound = 0
											;n\Sound = LoadSound_Strict("SFX\Character\MTF2\173\Cont"+Rand(1,6)+".ogg")
											;PlayMTFSound(n\Sound, n)
                ...
                If temp>False Then
					...
					;	If temp < 2
					;		If n\Sound <> 0 Then FreeSound_Strict n\Sound : n\Sound = 0
					;		n\Sound = LoadSound_Strict("SFX\Character\MTF2\ThereHeIs"+Rand(1,6)+".ogg")
					;		PlayMTFSound(n\Sound, n)
					;	EndIf
					;Else
						;If temp = True
						;	If n\Sound <> 0 Then FreeSound_Strict n\Sound : n\Sound = 0
						;	n\Sound = LoadSound_Strict("SFX\Character\MTF2\Stop"+Rand(1,6)+".ogg")
						;	PlayMTFSound(n\Sound, n)
						;ElseIf temp = 2
						;	;If n\Sound <> 0 Then FreeSound_Strict n\Sound : n\Sound = 0
						;	;n\Sound = MTFSFX(Rand(0,3))
			...
			Case 1 ;searching for player
						...
						If n\Reload =< 0 And KillTimer = 0 Then
							If EntityVisible(n\Collider, Camera) Then
								...
								If angle < 5 Or angle > 355 Then 
									...
									DeathMSG="SUBJECT: WALKER | STATUS: DEAD | "
									DeathMSG=DeathMSG+"POST-MORTEM: Subject died of bloodloss following assault by Black Operations."
									
									;player killed -> "target terminated"
									If prev => 0 And KillTimer < 0 Then
										DeathMSG="SUBJECT: WALKER | STATUS: DEAD | "
										DeathMSG=DeathMSG+"POST-MORTEM: Terminated by Black Operations."
									;	PlayMTFSound(LoadTempSound("SFX\Character\MTF2\Targetterminated"+Rand(1,5)+".ogg"),n)
									EndIf
								EndIf	
							EndIf
						EndIf
                ...
                Else
					...
					;	If Rand(2)=1 Then 
					;		PlayMTFSound(LoadTempSound("SFX\Character\MTF2\Searching"+Rand(1,6)+".ogg"),n)
					;	EndIf
					;EndIf
						...
						If curr173Dist<tempDist Then
							If n\MTF2Leader = Null Then
								...
								If n\State3>=70.0*15.0 Then
									...
									;If n\Sound <> 0 Then FreeSound_Strict n\Sound : n\Sound = 0
									;n\Sound = LoadSound_Strict("SFX\Character\MTF2\173\Box"+Rand(1,4)+".ogg")
									;PlayMTFSound(n\Sound, n)
								EndIf
							EndIf
							PositionEntity n\obj,EntityX(Curr173\Collider,True),EntityY(Curr173\Collider,True),EntityZ(Curr173\Collider,True),True
			...
			If WearingVest>0 Then
				If WearingVest = 1 Then
					Select Rand(8)
						Case 1,2,3,4,5
							BlurTimer = 500
							Stamina = 0
							ShotMessageUpdate = "A bullet penetrated your suit, making you gasp."
							If HEV_HP = 0
				...
				Else
					If Rand(10)=1 Then
						BlurTimer = 500
						Stamina = Stamina - 1
						ShotMessageUpdate = "A bullet hit your chest. The HEV Suit absorbed some of the damage."
						If HEV_HP = 0
						...
						EndIf
					Else
						ShotMessageUpdate = "A bullet hit your chest. The HEV Suit absorbed most of the damage."
						If HEV_HP = 0
						    Injuries = Injuries + Rnd(0.1,0.5)
						Else
						    DamageHEV(5)
...
Function Console_SpawnNPC(c_input$, c_state$ = "")
	...
	Select c_input$ 
		...
		Case "173", "robot", "bmr500"
			...
			consoleMSG = "BMR-500 spawned."
			
		Case "372", "aliencontroller", "controller"
			n.NPCs = CreateNPC(NPCtype372, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Alien Controller spawned."
			
		Case "513-1", "5131", "gman"
			n.NPCs = CreateNPC(NPCtype5131, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "G-Man spawned."
			
		Case "860-2", "8602", "gargantua", "garg"
			CreateConsoleMsg("This entity cannot be spawned with the console. Sorry!", 255, 0, 0)
			
		Case "939", "bullchicken", "bullsquid"
			CreateConsoleMsg("This entity cannot be spawned with the console. Sorry!", 255, 0, 0)

		Case "966", "shocktrooper", "strooper"
			n.NPCs = CreateNPC(NPCtype966, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Shock Trooper spawned."
			
		Case "1048-a", "1048a", "turret"
			CreateConsoleMsg("This entity cannot be spawned with the console. Sorry!", 255, 0, 0)
			
		Case "1499-1", "14991", "antlion"
			n.NPCs = CreateNPC(NPCtype1499, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Antlion Soldier spawned."
			
		Case "classd", "d", "player", "hev"
			n.NPCs = CreateNPC(NPCtypeD, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "HEV Subject spawned."
			
		Case "classd2", "d1", "gordon"
			n.NPCs = CreateNPC(NPCtypeD1, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Male HEV Subject spawned."
			
		Case "classd3", "d2", "gina"
			n.NPCs = CreateNPC(NPCtypeD2, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Female HEV Subject spawned."
			
		Case "guard", "barney"
			n.NPCs = CreateNPC(NPCtypeGuard, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Guard spawned."
			
		Case "mtf", "humangrunt", "hgrunt", "hecu", "soldier"
			n.NPCs = CreateNPC(NPCtypeMTF, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "HECU Marine spawned."
		...
		Case "tentacle", "tree"
			...
			consoleMSG = "Xenian Tree spawned."
			
		Case "clerk", "otis", "guard2"
			n.NPCs = CreateNPC(NPCtypeClerk, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Otis spawned."
			
		Case "457", "kingpin"
		    CreateConsoleMsg("This entity cannot be spawned with the console. Sorry!", 255, 0, 0)

        Case "049-3", "0493", "049zombie2", "zombiescientist"
			n.NPCs = CreateNPC(NPCtypeZombie2, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			n\State = 1
			consoleMSG = "Zombie Scientist spawned."

        Case "mtf2", "humanassassin", "hassassin", "blackops", "ops"
			n.NPCs = CreateNPC(NPCtypeMTF2, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Black Ops Assassin spawned."
			
		Case "scientist", "walter"
			n.NPCs = CreateNPC(NPCtypeScientist, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Scientist spawned."
			
		Case "franklin", "scientist2", "einstein"
			n.NPCs = CreateNPC(NPCtypeFranklin, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Scientist spawned."	
			
		Case "106victim", "scientist3", "luther"
			n.NPCs = CreateNPC(NPCtype106Victim, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Scientist spawned."
			
	    Case "janitor", "scientist4", "slick"
			n.NPCs = CreateNPC(NPCtypeJanitor, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Scientist spawned."
			
		Case "vehicle", "loader", "mtm8"
			n.NPCs = CreateNPC(NPCtypeVehicle, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			n\State = 0
			consoleMSG = "Loader spawned."	
...
;~B#1C9#12B6#1350#13E9#152B#1572#172D#1789
;~C#Blitz3D