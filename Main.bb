;SCP - Containment Breach Half-Life Resonance Cascade Mod

;    The game is based on the works of the SCP Foundation community (http://www.scp-wiki.net/).
;    The mod is based on Half-Life video game series.
...
Global ConsoleVersion% = GetINIInt(OptionFile, "console", "version")
...
Const HIT_MAP% = 1, HIT_PLAYER% = 2, HIT_ITEM% = 3, HIT_APACHE% = 4, HIT_178% = 5, HIT_DEAD% = 6, HIT_LADDER% = 7
SeedRnd MilliSecs()
...
AppTitle "Half-Life Resonance Cascade v"+VersionNumber
...
Font1% = AALoadFont("GFX\font\trebuc\Trebuchet MS.ttf", Int(24 * (GraphicHeight / 1024.0)), 0,0,0)
Font2% = AALoadFont("GFX\font\trebucbd\Trebuchet MS.ttf", Int(72 * (GraphicHeight / 1024.0)), 0,0,0)
Font3% = AALoadFont("GFX\font\DS-DIGI\DS-Digital.ttf", Int(18 * (GraphicHeight / 1024.0)), 0,0,0)
Font4% = AALoadFont("GFX\font\DS-DIGI\DS-Digital.ttf", Int(36 * (GraphicHeight / 1024.0)), 0,0,0)
Font5% = AALoadFont("GFX\font\segoesc\Segoe Script.ttf", Int(54 * (GraphicHeight / 1024.0)), 0,0,0)
...
Global GodMode%, NoClip%, NoClipSpeed# = 2.0, Cheats%
		...
		If KeyHit(28) And ConsoleInput <> "" Then
			...
			Select Lower(StrTemp)
				Case "help"
					...
					Select Lower(StrTemp)
						...
						Case "spawnitem"
							...
							CreateConsoleMsg("Example: spawnitem key6")
							CreateConsoleMsg("******************************")
						Case "spawn"
							...
							CreateConsoleMsg("008 / 0082 / 049 / 0492 / 0493 / 066 / 096 / 106 / 173")
							CreateConsoleMsg(" / 372 / 513-1 / 966 / 1499-1 / classd / scientist")
							CreateConsoleMsg(" / clerk / guard / mtf / mtf2 / apache / tentacle")
							CreateConsoleMsg("******************************")
				...
				Case "173speed"
					...
					CreateConsoleMsg("BMR-500's speed set to " + StrTemp)
					;[End Block]
				Case "106speed"
					;[Block]
					StrTemp$ = Lower(Right(ConsoleInput, Len(ConsoleInput) - Instr(ConsoleInput, " ")))
					Curr106\Speed = Float(StrTemp)
					CreateConsoleMsg("Alien Slave's speed set to " + StrTemp)
					;[End Block]
				Case "173state"
					;[Block]
					CreateConsoleMsg("BMR-500")
				...
				Case "newyear" 
					...
					If NYTex Then
						...
						EntityTexture Curr173\obj, tex, 0, 0
						FreeTexture tex
						CreateConsoleMsg("NEW YEAR MOD ON")
					Else
						tex2 = LoadTexture_Strict("GFX\npcs\173texture.jpg", 1)
						EntityTexture Curr173\obj, tex2, 0, 0
						FreeTexture tex2
						CreateConsoleMsg("NEW YEAR MOD OFF")
					EndIf
					;[End Block]	
				Case "sanic"
					;[Block]
				...
				Case "scp-420-j","420","weed"
					...
					For i = 1 To 20
						If Rand(2)=1 Then
							it.Items = CreateItem("Rad Bottle","420", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
						Else
							it.Items = CreateItem("Adrenaline Bottle","420s", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
						EndIf
						EntityType (it\collider, HIT_ITEM)
					Next
				...
				Case "096state"
					...
					For n.NPCs = Each NPCs
						...
						EndIf
					Next
					CreateConsoleMsg("Alien Grunt has not spawned.")
					;[End Block]
				Case "debughud"
					;[Block]
					StrTemp$ = Lower(Right(ConsoleInput, Len(ConsoleInput) - Instr(ConsoleInput, " ")))
					...
					Select StrTemp
						Case "a"
							For e.Events = Each Events
								...
								EndIf
							Next
							CreateConsoleMsg("Exit A is now unlocked.")	
						Case "b"
							For e.Events = Each Events
								...
								EndIf
							Next	
							CreateConsoleMsg("Exit B is now unlocked.")	
						Default
							For e.Events = Each Events
								If e\EventName = "gateaentrance" Then
									e\EventState3 = 1
								...
								EndIf
							Next
							CreateConsoleMsg("Exits A and B are now unlocked.")	
					End Select
				...
				Case "kill","suicide"
					...
					DeathMSG = DeathMSG + "POST-MORTEM: Died by committing suicide."
					;[End Block]
				Case "playmusic"
					;[Block]
...
;					EndIf
				...
				Case "resethev" 
				    ;[Block]
				    If WearingVest = 1 Then HEV_HP = 100
				    If WearingVest = 2 Then HEV_HP = 200
				    HEV_DHP = 0
				    ;[End Block]
				Default
					;[Block]
...
Global ApacheSFX
Global CurrStepSFX
Dim StepSFX%(5, 2, 9) ;(normal/metal, walk/run, id)
	...
	ElseIf d\KeyCard < 0
		...
		If temp <> 0 Then
			...
			If (Instr(Msg,"You placed your")=0) Or (MsgTimer < 70*3) Then
				Msg = "DNA verified. Access granted."
			EndIf
			MsgTimer = 70 * 10
		Else
			If showmsg = True Then 
				PlaySound_Strict ScannerSFX2
				Msg = "DNA does not match known sample. Access denied."
				MsgTimer = 70 * 10
			EndIf
			Return			
		EndIf
...
Global IceOverlay%, IceTexture%

Global CrystalOverlay%, CrystalTexture%
	...
	Select e\EventName
		...
		Case "room2closets"
			...
			If e\EventState = 0
				If e\EventStr = "load0"
					QuickLoadPercent = 10
					If e\room\NPC[0]=Null Then
						e\room\NPC[0] = CreateNPC(NPCtypeScientist, EntityX(e\room\Objects[0],True),EntityY(e\room\Objects[0],True),EntityZ(e\room\Objects[0],True))
					EndIf
					e\EventStr = "load1"
				...
				ElseIf e\EventStr = "load3"
					...
					If e\room\NPC[1]=Null Then
						e\room\NPC[1] = CreateNPC(NPCtypeJanitor, EntityX(e\room\Objects[1],True),EntityY(e\room\Objects[1],True),EntityZ(e\room\Objects[1],True))
					EndIf
		...
		Case "room049"
			...
			If e\EventState = 0 Then
				If e\EventStr = "load0"
					...
					e\EventStr = "load1"
				ElseIf e\EventStr = "load1"
					n.NPCs = CreateNPC(NPCtypeZombie2, EntityX(e\room\Objects[5],True),EntityY(e\room\Objects[5],True),EntityZ(e\room\Objects[5],True))
					PointEntity n\Collider, e\room\obj
		...
		Case "room205"
			...
			If e\EventState=0 Or e\EventStr <> "loaddone" Then
				If e\EventStr = "load0"
					e\room\Objects[3] = CopyEntity(o\NPCModelID[37])
					QuickLoadPercent = 10
					e\EventStr = "load1"
				ElseIf e\EventStr = "load1"
					e\room\Objects[4] = CopyEntity(o\NPCModelID[38])
					QuickLoadPercent = 20
					e\EventStr = "load2"
				ElseIf e\EventStr = "load2"
					e\room\Objects[5] = CopyEntity(o\NPCModelID[39])
					QuickLoadPercent = 30
					e\EventStr = "load3"
				ElseIf e\EventStr = "load3"
					e\room\Objects[6] = CopyEntity(o\NPCModelID[40])
					QuickLoadPercent = 40
					e\EventStr = "load4"
				ElseIf e\EventStr = "load4"
					QuickLoadPercent = 50
		...
		Case "room409"
		    ...
		    If e\EventState = 0 Then
	            If e\EventStr = "load0"
	                e\room\NPC[0] = CreateNPC(NPCtypeCorpse, EntityX(e\room\Objects[0], True), EntityY(e\room\Objects[0], True) + 0.5, EntityZ(e\room\Objects[0], True))
		...
		Case "room009"
		    ...
		    If e\EventState4 = 0 Then
                If e\EventStr = "load0"
                    e\room\NPC[0] = CreateNPC(NPCtypeCorpse, EntityX(e\room\Objects[0], True), EntityY(e\room\Objects[0], True), EntityZ(e\room\Objects[0], True))
                    SetNPCFrame e\room\NPC[0], 19
					e\room\NPC[0]\State=8
					TurnEntity e\room\NPC[0]\Collider, 0, e\room\angle + 360, 0
					e\room\NPC[0]\IsDead = True
		...
		Else
			...
			If EndingTimer < -1000 And EndingTimer > -2000
				...
				If AchievementsMenu=0 Then 
					...
					AAText x, y, "Aliens and other objects encountered: " +scpsEncountered
					...
					AAText x, y+80*MenuScale, "Items refined in the Non-Relay Teleport: " +RefinedItems			
...
Function InitCredits()
	...
	CreditsFont% = LoadFont_Strict("GFX\font\trebuc\Trebuchet MS.ttf", Int(21 * (GraphicHeight / 1024.0)), 0,0,0)
	CreditsFont2% = LoadFont_Strict("GFX\font\trebuc\Trebuchet MS.ttf", Int(35 * (GraphicHeight / 1024.0)), 0,0,0)
...
Function MovePlayer()
	...
	If SuperMan Then
		...
		If SuperManTimer > 70 * 50 Then
			DeathMSG = "''Is it a crime to go too fast?''"
			DeathMSG = DeathMSG + "~Vinny, 2014"
			Kill()
			ShowEntity Fog
		Else
			BlurTimer = 500		
	...
	If PlayerRoom\RoomTemplate\Name<>"pocketdimension" Then 
		If KeyDown(KEY_SPRINT) Then
			If Stamina < 5 Then
				temp = 0
				If WearingGasMask>0 Then temp=1
		...
		If (Not UnableToMove%) And ShouldEntitiesFall Then 
			If (EntityCollided(Collider,HIT_LADDER)<>0) And (KeyDown(KEY_UP) Or Crouch) Then
				...
				If KeyDown(KEY_UP) Then
					TranslateEntity Collider, 0, 0.035 * FPSfactor, 0
				EndIf
			Else						
...
Function MouseLook()
	...
	If WearingGasMask Or WearingHazmat Then
		If Wearing714 = False Then
			If WearingGasMask = 2 Or WearingHazmat = 2 Then
	...
	If WearingVest = 1 Then
        HEV_HP = 100 - HEV_DHP
        If HEV_HP > 100 Then HEV_HP = 100
        If HEV_HP =< 0 Then HEV_HP = 0
        If HEV_DHP =< 0 Then HEV_DHP = 0
        If HEV_DHP >= 100 Then HEV_DHP = 100
    ElseIf WearingVest = 2
        HEV_HP = 200 - HEV_DHP
        If HEV_HP > 200 Then HEV_HP = 200
        If HEV_HP =< 0 Then HEV_HP = 0
        If HEV_DHP =< 0 Then HEV_DHP = 0
        If HEV_DHP >= 200 Then HEV_DHP = 200
    Else
        HEV_HP = 0
    EndIf
...
Function DrawGUI()
	...
	If HUDenabled Then 
	    ...
	    If WearingVest > 0
		    ...
		    Rect(x, y, width, height, False)
		    If WearingVest = 1
		        For i = 1 To Int(HEV_HP/5)
			        DrawImage(BlinkMeterIMG, x + 3 + 10 * (i - 1), y + 3)
		        Next
		    ElseIf WearingVest = 2
			    For i = 1 To Int(HEV_HP/10)
			        DrawImage(BlinkMeterIMG, x + 3 + 10 * (i - 1), y + 3)
		        Next
            EndIf
		    Color 0, 0, 0
		    ...
		    If WearingVest = 1
		        ...
		        Else
		            Color 255, 0, 0
		        EndIf
	     	    AAText x+230,y-5,HEV_HP,2,0
	            Color 255, 160, 0
	            AAText x+250,y-5,"/100"
	        ElseIf WearingVest = 2
	            If HEV_HP > 30
		            Color 255, 160, 0
		        Else
		...
		If DebugHUD Then
			...
			For ev.Events = Each Events
				If ev\room = PlayerRoom Then
					...
					AAText x - 50, 250, "state3: " + ev\EventState4
			...
			AAText x - 50, 400, "Bloodloss: " + Bloodloss
			If Curr173 <> Null
			...
			EndIf
			If Curr106 <> Null
				AAText x - 50, 500, "SCP - 106 Position: (" + f2s(EntityX(Curr106\obj), 3) + ", " + f2s(EntityY(Curr106\obj), 3) + ", " + f2s(EntityZ(Curr106\obj), 3) + ")"
				AAText x - 50, 520, "SCP - 106 Idle: " + Curr106\Idle
				AAText x - 50, 540, "SCP - 106 State: " + Curr106\State
			EndIf
			offset% = 0
			For npc.NPCs = Each NPCs
				If npc\NPCtype = NPCtype096 Then
					AAText x - 50, 560, "SCP - 096 Position: (" + f2s(EntityX(npc\obj), 3) + ", " + f2s(EntityY(npc\obj), 3) + ", " + f2s(EntityZ(npc\obj), 3) + ")"
					AAText x - 50, 580, "SCP - 096 Idle: " + npc\Idle
					AAText x - 50, 600, "SCP - 096 State: " + npc\State
					AAText x - 50, 630, "SCP - 096 Speed: " + f2s(npc\currspeed, 5)
				EndIf
			Next
			If PlayerRoom\RoomTemplate\Name$ = "dimension1499"
	...
	If OtherOpen<>Null Then
		...
		x = GraphicWidth / 2 - (width * MaxItemAmount /3 + spacing * (MaxItemAmount / 3 - 1)) / 2
						...
						If (Not isEmpty) Then
							For z% = 0 To OtherSize - 1
								If OtherOpen\SecondInv[z]<>Null
									...
									If name$<>"25ct" And name$<>"cash" And name$<>"key" And name$<>"scp860" And name$<>"scp714" Then
										isEmpty=False
										Exit
									EndIf
								EndIf
			...
			Else
				If MouseSlot = 66 Then
					Select SelectedItem\itemtemplate\tempname
						...
						Case "scp1499"
							If Wearing1499 > 0 Then
								Msg = "Double click to use the gun for teleporting back to the facility."
								MsgTimer = 70*5
							Else
								DropItem(SelectedItem)
								SelectedItem = Null
				...
				Else
					...
					ElseIf Inventory(MouseSlot) <> SelectedItem
						Select SelectedItem\itemtemplate\tempname
							Case "paper","key1","key2","key3","key4","key5","key6","misc","oldpaper","badge","ticket","25ct","cash","key","scp860"
								;[Block]
								If Inventory(MouseSlot)\itemtemplate\tempname = "clipboard" Then
									...
									If (b<>"misc" And b<>"25ct" And b<>"cash" And b<>"key" And b<>"scp860" And b<>"scp714") Or (b2="Playing Card" Or b2="Mastercard") Then
										...
										For c% = 0 To Inventory(MouseSlot)\invSlots-1
											If (Inventory(MouseSlot)\SecondInv[c] = Null)
												If SelectedItem <> Null Then
													Inventory(MouseSlot)\SecondInv[c] = SelectedItem
													Inventory(MouseSlot)\state = 1.0
													If b<>"25ct" And b<>"cash" And b<>"key" And b<>"scp860"
			...
			Select SelectedItem\itemtemplate\tempname
				Case "nvgoggles"
					...
					If WearingHazmat=0 Then
						If WearingNightVision = 1 Then
							...
							PlaySound_Strict(LoadTempSound("SFX\Interact\UseNVG2.ogg"))
						Else
							...
							CameraFogFar = 30
							PlaySound_Strict(LoadTempSound("SFX\Interact\UseNVG1.ogg"))
						EndIf
						
						WearingNightVision = (Not WearingNightVision)
						If WearingHazmat > 0 Then Msg = "You need to take off the hazmat suit in order to put on the goggles."
					EndIf
					SelectedItem = Null
					MsgTimer = 70 * 5
					;[End Block]
				...
				Case "scp500"
					...
					If CanUseItem(False, False, True)
						...
						If Infect > 0 Then
							Msg = "Antidote engaged. Your nausea is fading."
						ElseIf I_409\Timer > 0 Then
						    Msg = "Antidote engaged. Your body is getting warmer and the crystals are receding."
						Else
							Msg = "Antidote engaged."
						EndIf
						MsgTimer = 70*7
							...
							Case 5
								...
								Else
									For r.Rooms = Each Rooms
										...
										EndIf
									Next
									Msg = "For some inexplicable reason... you find yourself in Xen."
									MsgTimer = 70*8
								EndIf
						End Select
						...
						If CanUseItem(False, True, True)
							...
							If SelectedItem\state = 100 Then
								...
								Else
									...
									PlaySound_Strict(LoadTempSound("SFX\Interact\UseMedkit.ogg"))
									MsgTimer = 70*5
									RemoveItem(SelectedItem)
								EndIf							
							EndIf
					...
					If SelectedItem\itemtemplate\img = 0 Then
						Select SelectedItem\itemtemplate\name
							Case "Burnt Note" 
								...
								Color 255,255,255
								SetBuffer BackBuffer()
							Case "Xenocephmacro Imperium Document"
							...
							Case "Floppy Disk"
								...
								If (SelectedItem\state = 0) Then
									Msg = Chr(34)+"Hey, I remember this game!"+Chr(34)
									MsgTimer = 70*10
						...
						Else
							Select Int(SelectedItem\state2)
								...
								Case 1 ;h?¤lytyskanava
									...
									strtemp = "        WARNING - DISASTER OF UNKNOWN TYPE          "
								...
								Case 2 ;scp-radio
									...
									strtemp = "        ERROR: COULD NOT CONNECT TO LOCAL CHANNEL          "
								...
								Case 3
									...
									strtemp = "             EMERGENCY CHANNEL - RESERVED FOR COMMUNICATION IN THE EVENT OF DISASTER         "
										...
										Select RadioState(3)
											...
											Case 2800
												If Not RadioState3(7) Then
													RadioCHN(3) = PlaySound_Strict(LoadTempSound("SFX\Character\MTF2\Random.ogg"))	
													RadioState(3) = RadioState(3)+1	
													RadioState3(7) = True
												EndIf	
										End Select
					...
					If WearingVest > 0
				        If WearingVest = 1
					        If HEV_HP < 100
								...
								HEV_DHP = HEV_DHP - 25
								If HEV_HP > 75
									Msg = "Power level is 100 percent."
								Else
									Msg = "Power level is " + HEV_HP + " percent."
								EndIf
								MsgTimer = 70 * 5
								PlaySound_Strict(LoadTempSound("SFX\Interact\HEV\Battery.ogg"))
								RemoveItem(SelectedItem)
							Else
								Msg = "You don't need to charge your suit right now."
								MsgTimer = 70 * 5
							EndIf
						Else If WearingVest = 2
							If HEV_HP < 200
								HEV_HP = HEV_HP + 25
								HEV_DHP = HEV_DHP - 25
								If HEV_HP > 175
									Msg = "Power level is 200 percent."
								Else
									Msg = "Power level is " + HEV_HP + " percent."
								EndIf
								MsgTimer = 70 * 5
						...
						EndIf
					Else
						Msg = "You can't use this item until you're not in HEV Suit."
						MsgTimer = 70 * 5
					EndIf
					;[End Block]
				Case "420"
					...
					If CanUseItem(False,False,True)
						If Wearing714=1 Then
							Msg = Chr(34) + "Wait... Why it doesn't work?" + Chr(34)
						Else
							Msg = Chr(34) + "Antitoxin engaged. You feel better." + Chr(34)
				...
				Case "420s"
					...
					If CanUseItem(False,False,True)
						...
						Else
							DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
							DeathMSG = DeathMSG + "POST-MORTEM: Subject was killed by huge adrenaline shot."
							Msg = Chr(34) + "Damn, that's strong... I need to relax..." + Chr(34)
							KillTimer = -1						
						EndIf
						MsgTimer = 70 * 6
						RemoveItem(SelectedItem)
				...
				Case "scp714"
					;[Block]
					If Wearing714=1 Then
						Msg = "Memetic shield deactivated."
						Wearing714 = False
					Else
						GiveAchievement(Achv714)
						Msg = "Memetic shield activated."
						Wearing714 = True
					EndIf
					MsgTimer = 70 * 5
					SelectedItem = Null	
				...
				Case "vest","finevest"
					...
					If SelectedItem\state=100 Then
						If WearingVest>0 Then
							Msg = "You removed the suit."
							WearingVest = False
							DropItem(SelectedItem)
						Else
							If SelectedItem\itemtemplate\tempname="vest" Then
								Msg = "You put on the suit and feel slightly encumbered."
								WearingVest = 1
								PlaySound_Strict(LoadTempSound("SFX\Interact\HEV\Greeting.ogg"))
							Else
								Msg = "You put on the suit and feel heavily encumbered."
								WearingVest = 2
								PlaySound_Strict(LoadTempSound("SFX\Interact\HEV\Greeting.ogg"))
							EndIf
							If SelectedItem\itemtemplate\sound <> 66 Then PlaySound_Strict(PickSFX(SelectedItem\itemtemplate\sound))
					...
					If WearingHazmat = 0 Then
						...
						Else
							If SelectedItem\itemtemplate\tempname = "supergasmask"
								Msg = "You put on the helmet and you can breathe easier."
							Else
						...
						EndIf
					    If WearingHazmat > 0 Then Msg = "You need to take off the hazmat suit in order to put on the helmet."
					EndIf
					SelectedItem = Null
					MsgTimer = 70 * 5
					;[End Block]
							...
							If SelectedItem\itemtemplate\name = "S-NAV Navigator Ultimate" And (MilliSecs2() Mod 600) < 400 Then
								If Curr173<>Null Then
									...
									If dist < 8.0 * 4 Then
										...
										AAText(x - width / 2 + 10, y - height / 2 + 30, "BMR-500")
								...
								If Curr106<>Null Then
									...
									If dist < 8.0 * 4 Then
										...
										AAText(x - width / 2 + 10, y - height / 2 + 30 + (20*SCPs_found), "ALIEN SLAVE")
										SCPs_found% = SCPs_found% + 1
									EndIf
								EndIf
								If Curr096<>Null Then 
									...
									If dist < 8.0 * 4 Then
										Color 100, 0, 0
										Oval(x - dist * 1.5, y - 7 - dist * 1.5, dist * 3, dist * 3, False)
										AAText(x - width / 2 + 10, y - height / 2 + 30 + (20*SCPs_found), "ALIEN GRUNT")
										SCPs_found% = SCPs_found% + 1
									EndIf
								EndIf
									...
									If np\NPCtype = NPCtype049
										...
										If dist < 8.0 * 4 Then
											If (Not np\HideFromNVG) Then
												Color 100, 0, 0
												Oval(x - dist * 1.5, y - 7 - dist * 1.5, dist * 3, dist * 3, False)
												AAText(x - width / 2 + 10, y - height / 2 + 30 + (20*SCPs_found), "GONOME")
												SCPs_found% = SCPs_found% + 1
											EndIf
										EndIf
										Exit
								...
								If PlayerRoom\RoomTemplate\Name = "room895" Then
									If CoffinDistance < 8.0 Then
										...
										Color 100, 0, 0
										Oval(x - dist * 1.5, y - 7 - dist * 1.5, dist * 3, dist * 3, False)
										AAText(x - width / 2 + 10, y - height / 2 + 30 + (20*SCPs_found), "REACTOR 1386")
									EndIf
								EndIf
							End If
				...
				Case "scp1499"
					...
					If SelectedItem\state=100 Then
						If Wearing1499>0 Then
							...
							;DropItem(SelectedItem)
							;If SelectedItem\itemtemplate\sound <> 66 Then PlaySound_Strict(PickSFX(SelectedItem\itemtemplate\sound))
							If SelectedItem\itemtemplate\sound <> 66 Then PlaySound_Strict(LoadTempSound("SFX\SCP\1499\DisplacerUse.ogg"))
						Else
				...
				Case "badge"
					...
					If SelectedItem\state = 0 Then
						...
						Select SelectedItem\itemtemplate\name
							Case "Old Badge"
								PlaySound_Strict LoadTempSound("SFX\SCP\1162\NostalgiaCancer"+Rand(6,10)+".ogg")
								Msg = Chr(34)+"Who is this guy? What is his badge doing there?"+Chr(34)
								MsgTimer = 70*10
						End Select
				...
				Case "key"
					...
					If SelectedItem\state = 0 Then
						PlaySound_Strict LoadTempSound("SFX\SCP\1162\NostalgiaCancer"+Rand(6,10)+".ogg")
						Msg = Chr(34)+"Is this the key from Kleiner's office? Geez, he lost it again..."+Chr(34)
						MsgTimer = 70*10						
					EndIf
					...
					If SelectedItem\state = 0
						Select SelectedItem\itemtemplate\name
							Case "Poster"
								...
								Msg = Chr(34)+"Damn you, Gaben!"+Chr(34)
								MsgTimer = 70*10
								PlaySound_Strict LoadTempSound("SFX\SCP\1162\NostalgiaCancer"+Rand(6,10)+".ogg")
								SelectedItem\state = 1
						End Select
				...
				Case "cash"
					...
					Msg = Chr(34)+"Hmm, it looks like this banknote is fake. Is this someone's prank?"+Chr(34)
					MsgTimer = 70*10
				...
				Case "scp427"
					...
					If I_427\Using=1 Then
						Msg = "Mobile recharger deactivated."
						I_427\Using = False
					Else
						GiveAchievement(Achv427)
						Msg = "Mobile recharger activated."
						I_427\Using = True
					EndIf
					MsgTimer = 70 * 5
					SelectedItem = Null
				...
				Case "p90"
					;[Block]
					Msg = Chr(34)+"I don't have ammo. I think it will not be useful to me."+Chr(34)
					MsgTimer = 70*5
					;[End Block]
				Default
					;[Block]
	...
	For it.Items = Each Items
		If it<>SelectedItem
			Select it\itemtemplate\tempname
				Case "firstaid","finefirstaid","firstaid2","vest","finevest","hazmatsuit","hazmatsuit2","hazmatsuit3","scp1499"
					it\state = 0
			End Select
		EndIf
	Next
...
Function DrawMenu()
	...
	If MenuOpen Then
		...
		Else
			AASetFont Font2
			AAText(x, y-(122-45)*MenuScale, "GAME OVER",False,True)
			AASetFont Font1
		End If		
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu > 0 And QuitMSG <= 0 And KillTimer >= 0
			If DrawButton(x + 101 * MenuScale, y + 420 * MenuScale, 230 * MenuScale, 60 * MenuScale, "Back") Then
				AchievementsMenu = 0
				OptionsMenu = 0
				QuitMSG = 0
				MouseHit1 = False
			...
			Select OptionsMenu
				...
				Case 3 ;Controls
					...
					AAText(x, y + 200 * MenuScale, "Take Screenshot")
					InputBox(x + 200 * MenuScale, y + 200 * MenuScale,100*MenuScale,20*MenuScale,KeyName(Min(KEY_SCREENSHOT,210)),13)
					If CanOpenConsole	
					    AAText(x, y + 220 * MenuScale, "Open/Close Console")
					    InputBox(x + 200 * MenuScale, y + 220 * MenuScale,100*MenuScale,20*MenuScale,KeyName(Min(KEY_CONSOLE,210)),12)
					EndIf
				...
				Case 4 ;Advanced
					...
					If CanOpenConsole
					    ...
					    If ConsoleVersion = 1 Then
							AAText(x + 310 * MenuScale, y, "New")
					    Else
					        AAText(x + 310 * MenuScale, y, "Old")
					    EndIf    
					    If MouseOn(x+270*MenuScale,y+MenuScale,20*MenuScale,20*MenuScale)
...
Const MaxNPCModelID = 46
Type Objects
    Field NPCModelID%[MaxNPCModelID-1]
End Type
...
Function LoadEntities()
	...
	HEVIcon% = LoadImage_Strict("GFX\HEVicon.png")
	...
	IceTexture = LoadTexture_Strict("GFX\AcidOverlay.png",1)
	IceOverlay = CreateSprite(ark_blur_cam)
	ScaleSprite(IceOverlay, Max(GraphicWidth / 1024.0, 1.0), Max(GraphicHeight / 1024.0, 1.0))
	EntityTexture(IceOverlay, IceTexture)
	EntityBlend (IceOverlay, 3)
	EntityFX(IceOverlay, 1)
	EntityOrder IceOverlay, -1001
	MoveEntity(IceOverlay, 0, 0, 1.0)
	HideEntity(IceOverlay)
	
	CrystalTexture = LoadTexture_Strict("GFX\CrystalOverlay.png", 1)
	CrystalOverlay = CreateSprite(ark_blur_cam)
	ScaleSprite(CrystalOverlay, Max(GraphicWidth / 1024.0, 1.0), Max(GraphicHeight / 1024.0, 1.0))
	EntityTexture(CrystalOverlay, CrystalTexture)
	EntityBlend (CrystalOverlay, 3)
	EntityFX(CrystalOverlay, 1)
	EntityOrder CrystalOverlay, -1001
	MoveEntity(CrystalOverlay, 0, 0, 1.0)
	HideEntity(CrystalOverlay)
    ...
    o\NPCModelID[9] = LoadMesh_Strict("GFX\npcs\173_2.b3d")
	...
	o\NPCModelID[35] = LoadAnimMesh_Strict("GFX\npcs\scientist.b3d")

	o\NPCModelID[36] = LoadAnimMesh_Strict("GFX\npcs\zombie2.b3d")
	
	o\NPCModelID[37] = LoadAnimMesh_Strict("GFX\npcs\205_demon1.b3d")
					
	o\NPCModelID[38] = LoadAnimMesh_Strict("GFX\npcs\205_demon2.b3d")

	o\NPCModelID[39] = LoadAnimMesh_Strict("GFX\npcs\205_demon3.b3d")

	o\NPCModelID[40] = LoadAnimMesh_Strict("GFX\npcs\205_woman.b3d")
	
	o\NPCModelID[41] = LoadAnimMesh_Strict("GFX\npcs\457.b3d")
	
	o\NPCModelID[42] = LoadAnimMesh_Strict("GFX\npcs\MTF2.b3d")
	
	o\NPCModelID[43] = LoadAnimMesh_Strict("GFX\npcs\vehicle.b3d")
	
	o\NPCModelID[44] = LoadAnimMesh_Strict("GFX\npcs\classd2.b3d")
	
	o\NPCModelID[45] = LoadAnimMesh_Strict("GFX\npcs\classd3.b3d")
	 
	For i = 0 To MaxNPCModelID-1
...
Function InitNewGame()
	...
	For r.Rooms = Each Rooms
		...
		If (r\RoomTemplate\Name = "start" And IntroEnabled = False) Then 
			...
			PlayerRoom = r
			it = CreateItem("HEV Suit", "vest", 1, 1, 1)
			it\Picked = True
			it\Dropped = -1
			...
			EntityParent(it\collider, 0)
			ItemAmount = ItemAmount + 1
			it = CreateItem("Assignment Sheet", "paper", 1, 1, 1)
			it\Picked = True
			it\Dropped = -1
			it\itemtemplate\found=True
			Inventory(1) = it
			...
			EntityParent(it\collider, 0)
			ItemAmount = ItemAmount + 1
			it = CreateItem("BMR-500 Document", "paper", 1, 1, 1)
			it\Picked = True
			it\Dropped = -1
			it\itemtemplate\found=True
			Inventory(2) = it
			...
			EntityParent(it\collider, 0)
			ItemAmount = ItemAmount + 1
			it = CreateItem("Walker's Badge", "badge", 1, 1, 1)
			it\Picked = True
			it\Dropped = -1
			it\itemtemplate\found=True
			Inventory(3) = it
...
Function NullGame(playbuttonsfx%=True)
	...
	Local o.Objects = First Objects
	KillSounds()
	If playbuttonsfx Then PlaySound_Strict ButtonMenuSFX
...
Function Use914(item.Items, setting$, x#, y#, z#)
	...
	Select item\itemtemplate\name
		Case "HEV Helmet", "Heavy HEV Helmet"
			Select setting
				Case "rough", "coarse"
				...
				Case "fine", "very fine"
					it2 = CreateItem("HEV Helmet", "supergasmask", x, y, z)
					RemoveItem(item)
			End Select
		Case "Displacer Gun"
				Select setting
					...
					RemoveItem(item)
				Case "coarse"
					it2 = CreateItem("U-235", "ammop90", x, y, z)
					RemoveItem(item)
				Case "1:1"
					it2 = CreateItem("Tau Cannon", "p90", x, y, z)
					RemoveItem(item)
				Case "fine", "very fine"
					PositionEntity(item\collider, x, y, z)
					ResetEntity(item\collider)
		...
		Case "Tau Cannon"
			Select setting
				...
				Case "coarse"
					...
					RemoveItem(item)
				Case "1:1"
					it2 = CreateItem("Displacer Gun", "scp1499", x, y, z)
					RemoveItem(item)
				Case "fine", "very fine"
					PositionEntity(item\collider, x, y, z)
					ResetEntity(item\collider)
		...
		Case "HEV Suit"
			Select setting
				...
				Case "coarse"
					it2 = CreateItem("HEV Battery", "hevbat", x, y, z)
					RemoveItem(item)
				Case "1:1"
					PositionEntity(item\collider, x, y, z)
					ResetEntity(item\collider)
				Case "fine"
					it2 = CreateItem("Heavy HEV Suit", "finevest", x, y, z)
					RemoveItem(item)
				Case "very fine"
					it2 = CreateItem("Bulky HEV Suit", "veryfinevest", x, y, z)
					RemoveItem(item)
			End Select
		Case "Clipboard"
			Select setting
		...
		Case "Night Vision Goggles"
			Select setting
				...
				Case "very fine"
					...
					RemoveItem(item)
			End Select
		Case "Xenium Metal Panel", "Xenium Metal"
			Select setting
				Case "rough", "coarse"
					it2 = CreateItem("Xenium Metal", "scp148ingot", x, y, z)
					RemoveItem(item)
				Case "1:1", "fine", "very fine"
					it2 = Null
					For it.Items = Each Items
					...
					If it2<>Null Then
						Select it2\itemtemplate\tempname
							Case "gasmask", "supergasmask"
								...
								it2 = CreateItem("Heavy HEV Helmet", "gasmask3", x, y, z)
							Case "vest"
								RemoveItem (it2)
								RemoveItem(item)
								it2 = CreateItem("Heavy HEV Suit", "finevest", x, y, z)
							Case "hazmatsuit","hazmatsuit2"
								RemoveItem (it2)
								RemoveItem(item)
								it2 = CreateItem("Heavy Cleansuit", "hazmatsuit3", x, y, z)
						End Select
					Else 
						If item\itemtemplate\name="Xenium Metal" Then
							it2 = CreateItem("Xenium Metal Panel", "scp148", x, y, z)
							RemoveItem(item)
						Else
							PositionEntity(item\collider, x, y, z)
							ResetEntity(item\collider)							
		...
		Case "Severed Hand", "Black Severed Hand"
			...
			End Select
			RemoveItem(item)
		Case "Medical Kit", "Modified Medical Kit"
			Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.005, z, 90, Rand(360), 0)
					d\Size = 0.12 : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
				If Rand(2)=1 Then
					it2 = CreateItem("Modified Medical Kit", "firstaid2", x, y, z)
				Else
				    it2 = CreateItem("Medical Kit", "firstaid", x, y, z)
				EndIf
				Case "fine"
					it2 = CreateItem("Small Medical Kit", "finefirstaid", x, y, z)
				Case "very fine"
					it2 = CreateItem("Defective Medical Kit", "veryfinefirstaid", x, y, z)
			End Select
			RemoveItem(item)
		Case "Level 1 Key Card", "Level 2 Key Card", "Level 3 Key Card", "Level 4 Key Card", "Level 5 Key Card", "Key Card"
							...
							Select SelectedDifficulty\otherFactors
								Case EASY
									If Rand(0,((MAXACHIEVEMENTS-1)*3)-((CurrAchvAmount-1)*3))=0
										it2 = CreateItem("Administration Group Keycard", "key6", x, y, z)
									Else
										it2 = CreateItem("Mastercard", "misc", x, y, z)
									EndIf
								Case NORMAL
			...
			RemoveItem(item)
		Case "Administration Group Keycard"
			Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.005, z, 90, Rand(360), 0)
					d\Size = 0.07 : ScaleSprite(d\obj, d\Size, d\Size)
			...
			RemoveItem(item)
		Case "Playing Card", "Coin", "Banknote"
			Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.005, z, 90, Rand(360), 0)
					d\Size = 0.07 : ScaleSprite(d\obj, d\Size, d\Size)
		...
		Case "Mastercard"
			Select setting
				Case "rough"
					...
					d\Size = 0.07 : ScaleSprite(d\obj, d\Size, d\Size)
				Case "coarse"
					it2 = CreateItem("Coin", "25ct", x, y, z)
					Local it3.Items,it4.Items,it5.Items
					it3 = CreateItem("Coin", "25ct", x, y, z)
					it4 = CreateItem("Coin", "25ct", x, y, z)
					it5 = CreateItem("Coin", "25ct", x, y, z)
		...
		Case "Briefcase"
			Select setting
				Case "rough", "coarse"
					...
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1", "fine", "very fine"
					it2 = CreateItem("Briefcase", "scp513", x, y, z)
		...
		Case "Rad Bottle"
			Select setting
				Case "rough", "coarse"			
					...
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("Antidote Bottle", "scp500", x + 1.5, y + 0.5, z + 1.0)
				Case "fine"
					it2 = CreateItem("Adrenaline Bottle", "420s", x + 1.5, y + 0.5, z + 1.0)
				Case "very fine"
					it2 = CreateItem("Super Adrenaline Bottle", "420s", x + 1.5, y + 0.5, z + 1.0)
			End Select
		...
		Case "HEV Battery"
			Select setting
				Case "rough", "coarse"			
					...
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("9V Battery", "bat", x + 1.5, y + 0.5, z + 1.0)
				Case "fine", "very fine"
					it2 = CreateItem("Mobile Recharger", "scp427", x, y, z)
			End Select
		...
		Case "Cleansuit"
			Select setting
				Case "rough", "coarse"
					...
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("Cleansuit", "hazmatsuit", x,y,z)
				Case "fine"
					it2 = CreateItem("Cleansuit", "hazmatsuit2", x,y,z)
				Case "very fine"
					it2 = CreateItem("Cleansuit", "hazmatsuit2", x,y,z)
			End Select
				...
				Case "veryfinesyringe"
					Select setting
						Case "rough"
							...
							d\Size = 0.07 : ScaleSprite(d\obj, d\Size, d\Size)
						Case "coarse"
							it2 = CreateItem("Modified Medical Kit", "firstaid", x, y, z)
						Case "1:1", "fine", "very fine"
							it2 = CreateItem("Defective Medical Kit", "veryfinefirstaid", x, y, z)
					End Select
			End Select
		...
		Case "Antidote Bottle", "Upgraded Antidote"
			Select setting
				Case "rough"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.010, z, 90, Rand(360), 0)
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "coarse"
					...
					RemoveItem(item)
				Case "1:1"
					it2 = CreateItem("HEV Battery", "hevbat", x + 1.5, y + 0.5, z + 1.0)
					RemoveItem(item)
				Case "fine"
					...
					If (Not no427Spawn) Then
						it2 = CreateItem("Mobile Recharger", "scp427", x, y, z)
					Else
						it2 = CreateItem("Upgraded Antidote", "scp500death", x, y, z)
					EndIf
					RemoveItem(item)
				Case "very fine"
					it2 = CreateItem("Upgraded Antidote", "scp500death", x, y, z)
		...
		Case "Pill"
			Select setting
				...
				Case "fine"
					...
					RemoveItem(item)
				Case "very fine"
					it2 = CreateItem("Antidote Bottle", "scp500", x, y, z)
					RemoveItem(item)
			End Select
				...
				Case "paper"
					Select setting
						...
						Case "1:1"
							Select Rand(6)
								Case 1
									it2 = CreateItem("Xenotherium Subservilia Document", "paper", x, y, z)
								Case 2
									it2 = CreateItem("Announcement System Notice", "paper", x, y, z)
								Case 3
									it2 = CreateItem("BMR-500 Document", "paper", x, y, z)
								Case 4
									it2 = CreateItem("Lambda Reactor #1386 Protocol", "paper", x, y, z)
								Case 5
									it2 = CreateItem("Xenotaur Gargantua Document", "paper", x, y, z)
								Case 6
									it2 = CreateItem("Xen Survey Method #11", "paper", x, y, z)
							End Select
						Case "fine", "very fine"
	...
	If temp Then
		If MouseHit1 Then
			...
			If ytemp => 0 And ytemp < 5 Then
				If xtemp => 0 And xtemp < 10 Then PlaySound_Strict ButtonSFX3
			EndIf
...
Function Use427()
	...
	If I_427\Timer < 70*360
		If I_427\Using=True Then
			...
			If WearingVest > 0
				If WearingVest = 1
					If HEV_HP < 100
						HEV_HP = Min(HEV_HP + 5 * FPSfactor * 0.08, 100)
						HEV_DHP = Max(HEV_DHP - 5 * FPSfactor * 0.08, 0)
					EndIf
				Else If WearingVest = 2
					If HEV_HP < 200
						HEV_HP = Min(HEV_HP + 10 * FPSfactor * 0.08, 200)
						HEV_DHP = Max(HEV_DHP - 10 * FPSfactor * 0.08, 0)
					EndIf
				EndIf
			EndIf
			...
			If I_409\Timer > 0.0 Then
				I_409\Timer = Max(I_409\Timer - 0.001 * FPSfactor,0.0)
			EndIf
			For i = 0 To 5
	...
	Else
		...
		If I_427\Timer >= 70*420 Then
			Kill()
			DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
			DeathMSG = DeathMSG + "POST-MORTEM: Subject melted down while using the Mobile Charger too long, "
			DeathMSG = DeathMSG + "desprite the fact that he was wearing an HEV Suit."
...
Function Update409()
	...
	If I_409\Sound[0]=0 Then
		I_409\Sound[0] = LoadSound_Strict(DamageSFX(10))
	EndIf
	If I_409\Timer > 0 And I_409\Timer =< 100 Then
		ShowEntity CrystalOverlay
		
		If (Not I_427\Using=1 And I_427\Timer < 70*360) Then
			I_409\Timer = ((Min(I_409\Timer+FPSfactor*0.004,100)))
		EndIf	
		EntityAlpha CrystalOverlay, Min(((I_409\Timer*0.2)^2)/1000.0,0.5)
		...
		If I_409\Timer >= 96.9222
			DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
			DeathMSG = DeathMSG + "POST-MORTEM: Died by the effects of XF-4090 crystal."
			Kill()
        EndIf
    Else
		HideEntity CrystalOverlay
					...
					If teleportForInfect
						For r.Rooms = Each Rooms
							If r\RoomTemplate\Name="room008" Then
								...
								ResetEntity Collider
								r\NPC[0] = CreateNPC(NPCtypeFranklin, EntityX(r\Objects[6],True),EntityY(r\Objects[6],True)+0.2,EntityZ(r\Objects[6],True))
		...
		Else
			...
			If teleportForInfect
				...
				ElseIf Infect < 98.5
					...
					If temp < 94.7 Then 
						...
						DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
						DeathMSG = DeathMSG + "POST-MORTEM: Subject fell victim to Keplar Virus infection and was later terminated by military forces."
			...
			Else
				...
				If PlayerRoom\RoomTemplate\Name = "dimension1499" Then
					DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
					DeathMSG = DeathMSG + "POST-MORTEM: An HEV subject infected by Keplar Virus was found dead on the one of Xen islands "
					DeathMSG = DeathMSG + "with Displacer Cannon near his body."
				ElseIf PlayerRoom\RoomTemplate\Name = "gatea" Or PlayerRoom\RoomTemplate\Name = "exit1" Then
					DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
					DeathMSG = DeathMSG + "POST-MORTEM: An HEV subject infected by Keplar Virus has been detected near Exit "
					If PlayerRoom\RoomTemplate\Name = "gatea" Then
						DeathMSG = DeathMSG + "A"
					Else
						DeathMSG = DeathMSG + "B"
					EndIf
					DeathMSG = DeathMSG + ". Terminated by military forces."
				Else
					DeathMSG = ""
				EndIf
			EndIf
...
Function SaveOptionsINI()
	...
	PutINIValue(OptionFile, "binds", "Screenshot key", KEY_SCREENSHOT)
	
	PutINIValue(OptionFile, "console", "enabled", CanOpenConsole%)
	PutINIValue(OptionFile, "console", "auto opening", ConsoleOpening%)
	PutINIValue(OptionFile, "console", "version", ConsoleVersion%)
...
Function IsItemGoodFor1162(itt.ItemTemplates)
	...
	Select itt\tempname
		Case "key1", "key2", "key3"
			Return True
		Case "misc", "420", "hevbat"
			Return True
		Case "gasmask", "nvgoggles"
			Return True
		Case "radio", "18vradio"
			Return True
		Case "clipboard", "eyedrops"
			Return True
		Case "drawing"
...
Function CanUseItem(canUseWithHazmat%, canUseWithGasMask%, canUseWithEyewear%)
	If (canUseWithHazmat = False And WearingHazmat) Then
		...
		MsgTimer = 70*5
		Return False
	Else If (canUseWithGasMask = False And (WearingGasMask))
		Msg = "You can't use that item while wearing a helmet."
		MsgTimer = 70*5
		Return False
...
Global HEV_HP%, HEV_DHP%

Function DamageHEV(damage%)
	
	LightFlash = 0.2
	
	If damage% > 0 And damage% =< 20
	    Injuries = Injuries + Rnd(0.2)
	ElseIf damage% > 20 And damage% =< 50
	    Injuries = Injuries + Rnd(0.3)
	ElseIf damage% > 50
	    Injuries = Injuries + Rnd(0.4)
	EndIf
	
	HEV_HP = HEV_HP - Int(damage%)
		
	If WearingVest = 1
	    If HEV_HP > 100 Then HEV_HP = 100
	    If HEV_DHP > 100 Then HEV_HP = 100
	ElseIf WearingVest = 2
	    If HEV_HP > 200 Then HEV_HP = 200
	    If HEV_DHP > 200 Then HEV_HP = 200
	EndIf
	
    If HEV_HP =< 0 Then HEV_HP = 0
			
	HEV_DHP = HEV_DHP + Int(damage%)
