		...
		Select e\EventName
				...
				If e\EventState = 0 Then
					If PlayerRoom = e\room Then
						...
						If e\room\NPC[1] = Null Then
							e\room\NPC[1] = CreateNPC(NPCtypeFranklin, 0,0,0)
						EndIf
						...
						If e\room\NPC[0]=Null
							...
							e\room\NPC[4] = CreateNPC(NPCtypeD2, EntityX(e\room\Objects[3], True), 0.5, EntityZ(e\room\Objects[3], True))
							...
							e\room\NPC[5] = CreateNPC(NPCtypeD1, EntityX(e\room\Objects[4], True), 0.5, EntityZ(e\room\Objects[4], True))
					...
					If e\EventState3 > 0 Then
						...
						If e\EventState3 < 170 Then
							If e\EventState3 = 1.0 Then
								...
								PlaySound_Strict(IntroSFX(11))
								PlaySound2(IntroSFX(19), Camera, Collider, 8, 0.5)
								BlurTimer = 500
								ShowEntity Light
								EntityAlpha(Light, 0.5)
								
								CreateConsoleMsg("")
								CreateConsoleMsg("WARNING! Using a console commands or teleporting away from the intro scene may cause bugs or crashing.", 255, 0, 0)
								CreateConsoleMsg("")
							EndIf
							...
							If e\EventState3 < 15 Then
								...
								If e\EventState3 - FPSfactor / 30.0 < 3.7 And e\EventState3 > 3.7 Then PlaySound2(IntroSFX(20), Camera, Collider, 8, 0.5)
								...
								Else
									...
									DropSpeed = 0
									e\EventState3 = 15
									Msg = "Pick up the paper and wear your HEV Suit."
									MsgTimer = 70 * 7
								EndIf
						...
						ElseIf e\EventState3 < 900
							...
							If e\room\NPC[8] <> Null Or e\room\NPC[9] <> Null Or e\room\NPC[10] <> Null Then ;the 2 guards and ClassD
							...
							If e\room\NPC[11] <> Null Then ;vehicle
								If e\room\NPC[11]\State = 0 Then
									If Distance(EntityX(Collider), EntityZ(Collider), EntityX(e\room\obj, True) - 6688.0 * RoomScale, EntityZ(e\room\obj, True) - 1252.0 * RoomScale) < 2.5 Then
										e\room\NPC[11]\State = 1
									EndIf
								Else
									If EntityX(e\room\NPC[11]\Collider) > EntityX(e\room\obj,True)-1800.0 * RoomScale Then
							...
							If Distance(EntityX(e\room\NPC[3]\Collider), EntityZ(e\room\NPC[3]\Collider), EntityX(e\room\RoomDoors[2]\frameobj,True), EntityZ(e\room\RoomDoors[2]\frameobj, True)) < 4.5 And dist < 5.0 Then
								...
								e\room\NPC[2] = CreateNPC(NPCtypeD1, EntityX(e\room\Objects[2], True), 0.5, EntityZ(e\room\Objects[2], True))
								...
								PositionEntity e\room\NPC[6]\Collider, EntityX(e\room\obj,True)-1190.0 * RoomScale, 460.0 * RoomScale, EntityZ(e\room\obj, True) + 456.0 * RoomScale, True
								...
								Else
									...
									If (EntityDistance(Collider, e\room\NPC[3]\Collider) < 1.5) Then
										If EntityInView(e\room\NPC[3]\obj, Camera) Then
											...
											If MouseHit1 Then
												SelectedItem = CreateItem("BMR-500 Document", "paper", 0.0, 0.0, 0.0)
						...
						If e\room\NPC[7]<>Null Then
							...
							PositionEntity e\room\NPC[7]\Collider, EntityX(e\room\obj, True) - 3361.0 * RoomScale, -305.0 * RoomScale, EntityZ(e\room\obj, True) - 2165.0 * RoomScale
					...
					Else
						...
						If e\EventState = 0 Then
							If PlayerRoom = e\room Then
								...
								IntroSFX(19) = LoadSound_Strict("SFX\Room\Intro\Alarm.ogg")
								IntroSFX(20) = LoadSound_Strict("SFX\Room\Intro\Ew.ogg")
								...
								e\room\NPC[6] = CreateNPC(NPCtypeFranklin, e\room\x - 3712.0 * RoomScale, -0.3, e\room\z - 2208.0 * RoomScale)
								e\room\NPC[7] = CreateNPC(NPCtypeScientist, e\room\x - 3712.0 * RoomScale, -0.3, e\room\z - 2208.0 * RoomScale)
				...
				If PlayerRoom = e\room Then
					...
					If e\EventState3=0 Then
						If Rand(2)=1 Then
							...
							e\room\Objects[1]=CopyEntity(o\NPCModelID[22])
						...
						If (CoffinDistance < 4.0) And (hasBatteryFor895) And (Not Wearing714) Then
							...
							If Sanity < -1000 Then
								If WearingNightVision > 1
                                    ...
                                    DeathMSG = DeathMSG + "POST-MORTEM: Subject was found dead near the Lambda Reactor #1386 with a pair of digital night vision goggles. Possibly killed "
                                    DeathMSG = DeathMSG + "by the effects from [CLASSIFIED]."
								Else
									DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
                                    DeathMSG = DeathMSG + "POST-MORTEM: Subject was found dead near the Lambda Reactor #1386 with a pair of digital night vision goggles. Possibly killed "
                                    DeathMSG = DeathMSG + "by the effects from [CLASSIFIED]."
					...
					If e\EventState = 0 Then
						If e\room\dist < 8 And e\room\dist > 0 Then
							...
							Else
								...
								e\room\NPC[0]=CreateNPC(NPCtypeJanitor, EntityX(e\room\RoomDoors[0]\obj,True), 0.5, EntityZ(e\room\RoomDoors[0]\obj,True))
								...
								For i = 9 To 10
									...
									If dist<6.0 Then 
										If dist<100.0*RoomScale Then
											...
											If KillTimer = 0 Then
												DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
												DeathMSG = DeathMSG + "POST-MORTEM: Subject was killed by heavy impact."
						...
						If EntityY(Collider) < -1600.0 * RoomScale Then
							...
							Else ;the player is not at the exit, must've fallen down
								...
								If KillTimer => 0 Then 
									PlaySound_Strict HorrorSFX(8)
									DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
									DeathMSG = "POST-MORTEM: Subject fell off of the Xenian mainland and into space. Body has yet to be retrieved."
							...
							If EntityInView(e\room\Objects[0], Camera) Then
								...
								If MouseHit1 Then
									...
									If e\EventState2 < 2 Then
										If SelectedItem<>Null Then
											If SelectedItem\itemtemplate\tempname="25ct" Then
									...
									ElseIf e\EventState2 = 1 And (Not inserted) Then
										Using294=False
										Msg = "You need to insert another coin in order to use this machine."
										MsgTimer = 70*5
									ElseIf (Not inserted) Then
										Using294=False
										Msg = "You need to insert two coins in order to use this machine."
										MsgTimer = 70*5
									EndIf
								EndIf
							EndIf
					...
					Else
						...
						If BlinkTimer =-10 Then
							If e\EventState > 70*7.5 And e\EventState-FPSfactor =< 70*7.5 Then
								...
								For i = 0 To 1
									it2.Items = CreateItem("Coin","25ct",1,1,1)
									it2\Picked = True
									it2\Dropped = -1
			...
			Case "room2elevator2"
				...
				If e\room\dist < 8.0 And e\room\dist > 0 Then
					...
					e\room\NPC[0]=CreateNPC(NPCtypeGonzales, EntityX(e\room\Objects[0],True), 0.5, EntityZ(e\room\Objects[0],True))
			...
			Case "room2nuke"
				...
				If e\EventState3 = 0 Then
					n.NPCs = CreateNPC(NPCtypeScientist,EntityX(e\room\Objects[6],True),0.5,EntityZ(e\room\Objects[6],True))
								...
								If KillTimer => 0 Then 
									For i = 0 To 2
										If Distance(EntityX(Collider),EntityZ(Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True)) < 250.0*RoomScale Then
											...
											Kill()
											DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
											DeathMSG = DeathMSG + "POST-MORTEM: Subject was severely electrocuted by Tesla Coil. Died in seconds."
										EndIf
									Next
								EndIf
								...
								If e\room\grid\grid[ix+(iy*gridsz)]>0 Then
									...
									Select e\room\grid\grid[ix+(iy*gridsz)]
										...
										Case 7
											...
											it = CreateItem("Antidote Bottle","scp500",e\room\x+ix*2.0+(Cos(EntityYaw(tempInt,True))*(-208.0)*RoomScale)-(Sin(EntityYaw(tempInt,True))*1226.0*RoomScale),8.0+(80.0*RoomScale),e\room\z+iy*2.0+(Sin(EntityYaw(tempInt,True))*(-208.0)*RoomScale)+(Cos(EntityYaw(tempInt,True))*1226.0*RoomScale))
											EntityType (it\collider, HIT_ITEM)
					...
					If temp = True Then 
						...
						Select e\EventState 
							...
							Case 18
								...
								EntityType(it\collider, HIT_ITEM)
							Case 25
								e\room\NPC[0]=CreateNPC(NPCtypeCorpse, EntityX(e\room\obj)+Cos(e\room\angle-90)*760*RoomScale, 0.35, EntityZ(e\room\obj)+Sin(e\room\angle-90)*760*RoomScale)
							...
							If EntityVisible(e\room\Objects[2],Camera) Then 							
								...
								If dist < 0.6 Then
									...
									ElseIf e\EventState3>85*70 And e\EventState3-FPSfactor=<85*70	
										DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
										DeathMSG = DeathMSG + "POST-MORTEM: Subject died of bloodloss due to the effects of DD-7382."
										Kill()
									EndIf
				...
				If PlayerRoom = e\room Then
					...
					If e\EventState = 0 Then
						If EntityDistance(Collider, e\room\Objects[3])<2 Then
							n.NPCs = CreateNPC(NPCtype035, EntityX(e\room\Objects[4],True),0.5,EntityZ(e\room\Objects[4],True))
					...
					ElseIf e\EventState > 0
						If e\room\NPC[0]=Null Then
							For n.NPCs = Each NPCs
								If n\texture = "GFX\npcs\035victim.jpg" Then
									e\room\NPC[0]=n
									
									temp = e\room\NPC[0]\Frame
									
									FreeEntity e\room\NPC[0]\obj
									e\room\NPC[0]\obj = CopyEntity(o\NPCModelID[25])
									x = (GetINIFloat("DATA\NPCs.ini", "Scientist", "scale") / 2.4)
								...
								If EntityZ(Collider)>Min(EntityZ(e\room\Objects[7],True),EntityZ(e\room\Objects[8],True)) Then
									If EntityZ(Collider)<Max(EntityZ(e\room\Objects[7],True),EntityZ(e\room\Objects[8],True)) Then
										...
										If KillTimer < 0 And Bloodloss =>100 Then
											DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
											DeathMSG = DeathMSG + "POST-MORTEM: Subject killed by biohazardous Xenian environment."
										EndIf
									EndIf
								EndIf
							EndIf
						...
						ElseIf e\EventState > 0
							...
							If temp And x Then
								...
								If e\room\NPC[0]\Idle > 0
									...
									If i > 0
										...
										e\room\NPC[0]\SoundChn2 = LoopSound2(e\room\NPC[0]\Sound2,e\room\NPC[0]\SoundChn2,Camera,e\room\NPC[0]\Collider)
										e\room\NPC[0]\Idle = 0
							...
							ElseIf e\EventState < 70*240
								For n.NPCs = Each NPCs ;awake the zombies
									If n\NPCtype = NPCtypeZombie Or n\NPCtype = NPCtypeZombie2 And n\State = 0 Then
										n\State = 1
										SetNPCFrame(n, 155)
									EndIf
								Next
					...
					If e\EventState > -70*4 Then
						...
						ElseIf FallTimer < -230
							...
							If e\EventState =< -70*4 Then 
								...
								For n.NPCs = Each NPCs
									If n\NPCtype = NPCtypeZombie Or n\NPCtype = NPCtypeZombie2 Then
...
;						EndIf
					    ...
					    If EntityY(Collider) < -6900.0 * RoomScale
						    ...
						    ElseIf e\EventState = 1 ;bone broken
							    ...
							    If e\EventState3 >= 2500.0 Then
							    	If e\EventState2 = 1 And e\EventState3-FPSfactor < 2500.0 Then
						...
						Select e\EventState
							...
							Case 5
								...
								If e\EventState2 > 40*70 Then 
									e\EventState = e\EventState+1
									PlaySound2(LoadTempSound("SFX\SCP\205\ShadowVoice1.ogg"), Camera, e\room\Objects[6], 10, 1.0)
								EndIf	
							Case 7
								ShowEntity e\room\Objects[1]
								ShowEntity(e\room\Objects[6])
								...
								If e\EventState2 > 60*70 Then 
									e\EventState = e\EventState+1
									PlaySound2(LoadTempSound("SFX\SCP\205\ShadowVoice2.ogg"), Camera, e\room\Objects[6], 10, 1.0)
								EndIf
							Case 9
								ShowEntity e\room\Objects[1]
								ShowEntity(e\room\Objects[6])
								...
								If e\EventState2 > 80*70 Then 
									...
									PlaySound2(LoadTempSound("SFX\SCP\205\ShadowVoice3.ogg"), Camera, e\room\Objects[6], 10, 1.0)
								EndIf
							Case 11
								ShowEntity e\room\Objects[1]
								ShowEntity(e\room\Objects[6])
							...
							Case 67
								If (Rand(150)=1) Then
									...
									Else
									    DamageHEV(20)
									EndIf
									PlaySound_Strict (LoadTempSound("SFX\SCP\205\ShadowAttack.ogg"))
									CameraShake = 0.5
			...
			Case "room1123"
				;[Block]
				If PlayerRoom = e\room Then
				    If EntityDistance(Collider, e\room\Objects[3]) < 0.9 Then
					    If (Not Wearing714 = 1) Or (Not WearingHazmat=3) Or (Not WearingGasMask=3) Then
					        BlurTimer = 1000
					        CameraShake = 1
					        e\SoundCHN3 = LoopSound2(Ambient1123SFX, e\SoundCHN3, Camera, e\room\Objects[3], 4, 4.0)
					    EndIf
				    EndIf
					...
					If e\EventState = 1 Then						
						...
						e\room\NPC[0] = CreateNPC(NPCtypeNazi, EntityX(e\room\Objects[6], True), EntityY(e\room\Objects[6], True), EntityZ(e\room\Objects[6], True))
					...
					ElseIf e\EventState = 2
						...
						If e\EventState2 > 1000 Then
							...
							If e\EventState2 >= 1040 And e\EventState2 - FPSfactor < 1040 Then 
								PlaySound2(LoadTempSound("SFX\SCP\1123\Officer1.ogg"), Camera, e\room\NPC[0]\collider)
							ElseIf e\EventState2 >= 1400 And e\EventState2 - FPSfactor < 1400 Then 
								PlaySound2(LoadTempSound("SFX\SCP\1123\Officer2.ogg"), Camera, e\room\NPC[0]\collider)
							EndIf
							e\room\NPC[0]\State = 3
			...
			Case "room2testroom"
                ...
                If e\EventState = 0
                    If PlayerRoom = e\room Then
                        ;original 1048 event
						e\room\Objects[7] = CopyEntity(o\NPCModelID[22])
				...
				If (Not Contained106) Then
					...
					If e\EventState > 0 Then 
						...
						If e\EventState>200 Then
							If e\room\NPC[0]=Null Then
								e\room\NPC[0]=CreateNPC(NPCtype106Victim, EntityX(e\room\obj), 900.0*RoomScale, EntityZ(e\room\obj))
							...
							If e\EventState > 70 * 3 Then
								Select setting
									Case "rough"
										...
										If e\SoundCHN <> 0 Then StopChannel e\SoundCHN
										DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
										DeathMSG = DeathMSG + "POST-MORTEM: Subject appeared to commit suicide through the use of the Non-Relay Teleport."
									Case "coarse"
										BlinkTimer = -10
								...
								Select setting
									Case "coarse"
										...
										Else
										    DamageHEV(70)
										EndIf
										Msg = "You notice that you are started to bleeding heavily."
										MsgTimer = 70*8
									Case "1:1"
						...
						Case 2
							...
							If (prevFrame>646.0) Then
								If (PlayerRoom = e\room) Then
									...
									Msg = "A bullet hit your chest. The HEV Suit absorbed some of the damage."
									MsgTimer = 70.0 * 3.0
								Else
									e\EventState3 = 70*30
								EndIf
						...
						Case 3
							...
							If (e\EventState2>250.0 And e\EventState2-FPSfactor <= 250.0) Then
								Select Rand(3)
									...
									Case 2
										Msg = "You feel a burning pain in your stomach."
									Case 3
										Msg = "You feel a burning pain all over your body."
								End Select
							...
							Else If (e\EventState2>600.0 And e\EventState2-FPSfactor <= 600.0)
								Select Rand(4)
									...
									Case 2
										Msg = "Air escapes from your lungs."
									Case 3
										Msg = "You feel weak."
									Case 4
							...
							If (e\EventState2>70*15) Then
								DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
								DeathMSG = DeathMSG + "POST-MORTEM: Killed by automatic sentry turret in [REDACTED]."
								Kill()
								e\EventState = 4
								RemoveEvent(e)
							EndIf
				...
				If PlayerRoom = e\room
					...
					ElseIf e\EventState3 = 2.0
						...
						For itt.ItemTemplates = Each ItemTemplates
							If IsItemGoodFor1162(itt) And Rand(6)=1
								...
								If Injuries > 15
									DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
								    DeathMSG = DeathMSG + "POST-MORTEM: The subject was found dead in the Sector C Fuel Supply Room."
								    DeathMSG = DeathMSG + " An autopsy revealed that his right lung was missing. The cause of death"
								    DeathMSG = DeathMSG + " is still unknown."
					...
					ElseIf e\EventState3 >= 3.0
						...
						Select e\EventState
							...
							Case 2
								it = CreateItem("Poster","oldpaper",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True))
							Case 3
								it = CreateItem("Banknote","cash",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True))
							Case 4
								it = CreateItem("Floppy Disk","ticket",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True))
							Case 5
			...
			Case "room2sl"
				...
				For e2.Events = Each Events
					If e2\EventName = "room008"
						If e2\EventState = 2
							EntityTexture e\room\Objects[21],e\room\Textures[0],2
						Else
		    ...
		    Case "room009"
				...
				If PlayerRoom = e\room Then
				    ...
				    ElseIf e\EventState4 = 1.0 Then 
				  		e\SoundCHN = LoopSound2(AlarmSFX(5), e\SoundCHN, Camera, e\room\Objects[1], 8.0, 2.0)	
					    If e\EventState3 = 0.0 Then
					    ...
					    If e\EventState >= 300.0 And e\EventState3 = 0.0 Then
						    e\EventState3 = 1.0
						    PlaySound_Strict(LoadTempSound("SFX\SCP\009\Alert1.ogg"))
						    ;If Rand(2) = 1 Then
				    ...
				    If e\EventState >= 4290.0 And e\EventState3 = 1.0 Then
			            PlaySound_Strict(LoadTempSound("SFX\SCP\009\Alert2.ogg"))
					    e\EventState3 = 2.0
				    EndIf
				    ...
				    If e\EventState >= 4502.0 And e\EventState3 = 2.0 Then
					    If PlayerRoom = e\room Then
						    DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
							DeathMSG = DeathMSG + "POST-MORTEM: Subject was found dead in Sector B Hazardous Waste Room. "
							DeathMSG = DeathMSG + "Died due to prolonged exposure to radioactive waste."
					    ElseIf EntityDistance(Collider, e\room\obj) < 128.0 Then
						    DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
							DeathMSG = DeathMSG + "POST-MORTEM: Subject was found dead near the Sector B Hazardous Waste Room. "
							DeathMSG = DeathMSG + "Died due to prolonged exposure to radioactive waste."
					    Else
						    DeathMSG = "SUBJECT: WALKER | STATUS: DEAD | "
							DeathMSG = DeathMSG + "POST-MORTEM: Subject died due to prolonged exposure to radioactive waste."
					    EndIf
					    Kill()
					    e\EventState3 = 3.0
				    EndIf
			...
			Case "room457"
				;[Block]
				;e\EventState = 1/2/3/4/5: 	Target of SCP-457
				;x: Lever(?)
				...
				If PlayerRoom = e\room Then
					...
					If EntityY(Collider) < - 2400.0 * RoomScale
						...
						Else
							...
							If EntityDistance(e\room\Objects[4], e\room\NPC[0]\Collider) < 3.3 And e\room\RoomDoors[4]\open = False And e\room\RoomDoors[5]\open = False And e\room\RoomDoors[6]\open = False Then
							    Contained457 = True
							Else
							    Contained457 = False
							EndIf
			...
			Case "room4info"
				...
				If e\EventState = 0 Then
					...
					e\EventState = 1
				Else
					e\SoundCHN = LoopSound2(ScientistRadioSFX, e\SoundCHN, Camera, e\room\Objects[2], 4, 0.5)
			    EndIf
			    ;[End Block]
		End Select
...
Function UpdateEndings()
				...
				If PlayerRoom = e\room Then
					...
					If EntityY(Collider)>1040.0*RoomScale Then
						...
						If e\EventState = 0 Then
							...
							For n.NPCs = Each NPCs
								If n\NPCtype = NPCtypeMTF And n\NPCtype = NPCtypeMTF2
									RemoveNPC(n)
								EndIf
							Next
   ...
   CreateRoom("gatec", "GateC")
					...
					If e\EventState = 0 Then
						DrawLoading(0)
						e\room\Objects[0] = LoadMesh_Strict("GFX\MAP\Escape.cbr")
						PositionEntity e\room\Objects[0], EntityX(e\room\obj,True),EntityY(e\room\obj,True),EntityZ(e\room\obj,True)
						ScaleEntity (e\room\Objects[0],RoomScale,RoomScale,RoomScale)
						EntityType e\room\Objects[0], HIT_MAP
						EntityPickMode e\room\Objects[0], 3
...
If rt\roomname = "gatec" Then
    ;Create gate objects
    rt\Objects[0] = CreatePivot()
    PositionEntity(rt\Objects[0], r\x-4.0, r\y+0.5, r\z, True)
    EntityParent(rt\Objects[0], r\obj)
    
    rt\Objects[1] = CreatePivot()
    PositionEntity(rt\Objects[1], r\x-2.0, r\y+0.5, r\z+2.0, True)
    EntityParent(rt\Objects[1], r\obj)
    
    rt\Objects[2] = CreatePivot() ;evacuation point
    PositionEntity(rt\Objects[2], r\x+6.0, r\y, r\z, True)
    EntityParent(rt\Objects[2], r\obj)
    
    rt\Objects[3] = CreatePivot() ;position for 173 MTF kill scene
    PositionEntity(rt\Objects[3], r\x+2.0, r\y, r\z+3.0, True)
    EntityParent(rt\Objects[3], r\obj)
    
    rt\Objects[4] = CreatePivot() ;position for 173 player approach
    PositionEntity(rt\Objects[4], r\x+1.0, r\y, r\z+1.5, True)
    EntityParent(rt\Objects[4], r\obj)
    
    rt\Objects[5] = CreatePivot() ;position for evacuation scene
    PositionEntity(rt\Objects[5], r\x+5.0, r\y, r\z-1.0, True)
    EntityParent(rt\Objects[5], r\obj)
EndIf    
     CreateEvent("gatec", "gatec", 0)

    ;If the player enters Gate C for the first time
    If e\EventState = 0 Then
        ;Play ambient sound for the tunnel
        e\Sound = LoadSound_Strict("SFX\Ambient\Room\TunnelAmbienceC.ogg")
        e\SoundCHN = PlaySound2(e\Sound, Camera, e\room\obj, 10.0, 1.0)
        
        ;Spawn MTF units in the distance
        e\room\NPC[0] = CreateNPC(NPCtypeMTF, EntityX(e\room\Objects[0],True), EntityY(e\room\Objects[0],True), EntityZ(e\room\Objects[0],True))
        e\room\NPC[1] = CreateNPC(NPCtypeMTF, EntityX(e\room\Objects[1],True), EntityY(e\room\Objects[1],True), EntityZ(e\room\Objects[1],True))
    ...
    If e\EventState = 1 Then
        ;MTF spotted player
        If e\EventState2 = 0 And EntityDistance(Collider, e\room\NPC[0]\Collider) < 8.0 Then
            e\SoundCHN2 = PlaySound_Strict("SFX\Character\MTF\Spotted1.ogg")
            e\EventState2 = 1
        EndIf
        
        ;Check if SCP-173 is following the player
        has173 = False
        For n.NPCs = Each NPCs
            If n\NPCtype = NPCtype173 And EntityDistance(n\Collider, Collider) < 10 Then
                has173 = True
                Exit
            EndIf
        Next
        
        ;If the player is near the evacuation point
        If EntityDistance(Collider, e\room\Objects[2]) < 2.0 Then
            If has173 Then
                ;173 causes chaos ending
                e\EventState = 2
                e\EventState3 = 0
            Else
                ;Safe evacuation ending
                e\EventState = 3 
                e\EventState3 = 0
            EndIf
        EndIf
    ...
    ;173 chaos ending sequence
    If e\EventState = 2 Then
        e\EventState3 = e\EventState3 + fps\FPSfactor[0]
        
        ;Blink mechanic for 173
        BlinkTimer = -10
        
        ;First stage - 173 attacks MTF
        If e\EventState3 < 5.0 Then
            PositionEntity(Collider, EntityX(e\room\Objects[3],True), EntityY(e\room\Objects[3],True)+0.5, EntityZ(e\room\Objects[3],True))
            RotateEntity(Collider, 0, e\room\angle+90, 0)
            
            ;Death scream from MTF
            If e\EventState3 > 2.0 And e\EventState3 - fps\FPSfactor[0] <= 2.0 Then
                PlaySound_Strict("SFX\Character\MTF\Pain1.ogg")
            EndIf
        ;Second stage - 173 approaches player
        ElseIf e\EventState3 < 10.0 Then
            For n.NPCs = Each NPCs
                If n\NPCtype = NPCtype173 Then
                    PositionEntity(n\Collider, EntityX(e\room\Objects[4],True), EntityY(e\room\Objects[4],True)+0.2, EntityZ(e\room\Objects[4],True))
                    RotateEntity(n\Collider, 0, EntityYaw(Collider), 0)
                    Exit
                EndIf
            Next
        ;Final stage - 173 kills player
        ElseIf e\EventState3 < 15.0 Then
            BlinkTimer = 0
            BlinkEffect = 1.0
            If e\EventState3 > 12.0 Then
                Kill()
                PlaySound_Strict("SFX\Character\D9341\Neck.ogg")
                m_msg\DeathTxt = "Subject D-9341. Cause of death: Cervical fracture caused by SCP-173."
            EndIf
        EndIf
    EndIf
    ...
    If e\EventState = 3 Then
        ...
        ;First stage - MTF communicates about evacuation
        If e\EventState3 < 5.0 Then
            PositionEntity(Collider, EntityX(e\room\Objects[5],True), EntityY(e\room\Objects[5],True)+0.5, EntityZ(e\room\Objects[5],True))
            RotateEntity(Collider, 0, e\room\angle+180, 0)
            
            If e\EventState3 > 1.0 And e\EventState3 - fps\FPSfactor[0] <= 1.0 Then
                e\SoundCHN2 = PlaySound_Strict("SFX\Character\MTF\Franklin\Escort.ogg")
            EndIf
        ;Second stage - Fade to black with radio transmission
        ElseIf e\EventState3 < 15.0 Then
            CameraFogRange Camera, 0.5, 0.5+(e\EventState3-5.0)*2.0
            CameraFogColor Camera, 0, 0, 0
            
            If e\EventState3 > 7.0 And e\EventState3 - fps\FPSfactor[0] <= 7.0 Then
                e\SoundCHN = PlaySound_Strict("SFX\Ending\GateC\Evac_Radio_1.ogg")
            EndIf
            
            If e\EventState3 > 12.0 And e\EventState3 - fps\FPSfactor[0] <= 12.0 Then
                e\SoundCHN2 = PlaySound_Strict("SFX\Ending\GateC\Evac_Radio_2.ogg")
            EndIf
        ;Final stage - Show the ending text
        Else
            CameraFogRange Camera, 0.5, 20.0
